...
Component({
  ...
  lifetimes: {
    ready() {
      ...
      Promise.all([this.getDistance('dragBottom'), this.getDistance('drag')]).then((values) => {
        ...
        const itemHeight = ((endPos - startPos) / this.data.items.length).toFixed(2)// 每个列表项高度
        this.setData({
          startPos,
          endPos,
          itemHeight
  ...
  methods: {
    ...
    scrollTouchStart(event) {
      ...
      const obj = this.getPositionDomByXY(firstTouchPosition)
      this.setData({
        items: obj.items,
        movableView: {
          y: movableY,
    ...
    scrollTouchMove(event) {
      const {
        itemHeight, startPos, items, movableView
      } = this.data
      const choiceItem = movableView.item // 当前选中项
      let selectPosition = choiceItem.selectPosition
      // 选中项位置

      let selectIndex = choiceItem.selectIndex
      // 选中项索引值

      const moveDistance = event.changedTouches[0].pageY
      // 移动距离

      const movableX = 0
      // movable-area滑块位置处理

      const movableY = event.changedTouches[0].pageY - startPos - this.balanceDis// 纵坐标位置
    ...
    getPositionDomByXY(potions) {
      const {items, startPos, itemHeight} = this.data
      const y = potions.y - startPos // 起初触摸点相对于下拉列表位置
